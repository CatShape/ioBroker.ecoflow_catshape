...
async function updateCumulateDailyByTime(ecoflowCatshape, stringDeviceId, objSrcStateObj, numberNew, numberOld, boolOnline) {
    ...
    const numberBod = (new Date(dateNow.getFullYear(), dateNow.getMonth(), dateNow.getDate(), numberHour, numberMinute, numberSecond)).getTime();
    if ((numberTs < numberBod) && (numberNowMs >= numberBod)) {
        if (await ecoflowCatshape.objectExists(stringDeviceId + '.name')) {
            stringA = (await ecoflowCatshape.getStateAsync(stringDeviceId + '.name')).val + ': ';
        } else {
            stringA = '';
        }
