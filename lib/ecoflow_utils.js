...
async function updateDeviceStates(ecoflowCatshape, objData) {
    ...
    for (stringId in objA) {
        ...
        if (objStateObj.native.hasOwnProperty('ecoflowApi')) {
            if (objStateObj.native.ecoflowApi.hasOwnProperty('quotaValueKey')) {
                ...
                if (anyValue === null) {
                    if (!(['deviceName', 'productName'].includes(objStateObj.native.ecoflowApi.quotaValueKey))) {
                        ...
                        ecoflowCatshape.setStateChanged(stringIdA, JSON.stringify(objData), true);
                        if (ecoflowCatshape.objQuotaKeyNotFound.hasOwnProperty(stringId)) { // avoid logging the same warning repeatedly
                            ecoflowCatshape.log.debug(stringA);
                        } else {
                            ecoflowCatshape.objQuotaKeyNotFound[stringId] = true;
                            ecoflowCatshape.log.warn(stringA);
                        }
                    }
                } else {
                    delete ecoflowCatshape.objQuotaKeyNotFound[stringId];
    ...
    if (await ecoflowCatshape.objectExists(stringIdA)) {
        ...
        anyValue = getStateValueFromApi(objData, objA.native.ecoflowApi);
        if (anyValue !== null) {
            numberA = anyValue;
            objA = await ecoflowCatshape.getObjectAsync(stringDeviceId + '.' + objCurve.native.ampId);
            anyValue = getStateValueFromApi(objData, objA.native.ecoflowApi);
            if (anyValue !== null) {
                numberA = numberA - anyValue * objCurve.native.ampCorrectionFactor;
...
async function updateCumulateDailyByTime(ecoflowCatshape, stringDeviceId, objSrcStateObj, numberNew, numberOld, boolOnline) {
    ...
    let numberNewPerMs = dateNow.getTime();
    ...
    if (objCumulateDailyResetTime.hasOwnProperty('hour')) {
        numberHour = objCumulateDailyResetTime.hour;
    }
    if (objCumulateDailyResetTime.hasOwnProperty('minute')) {
        numberMinute = objCumulateDailyResetTime.minute;
    }
    if (objCumulateDailyResetTime.hasOwnProperty('second')) {
        numberSecond = objCumulateDailyResetTime.second;
    }
    const numberResetMs = (new Date(dateNow.getFullYear(), dateNow.getMonth(), dateNow.getDate(), numberHour, numberMinute, numberSecond)).getTime();
    if ((numberTs < numberResetMs) && (numberNewPerMs >= numberResetMs)) {
        if (boolOnline) {
            let numberReset = numberOld + (numberResetMs - numberTs) * ((numberNew - numberOld) / (numberNewPerMs - numberTs));
            numberVal = numberVal + (numberOld + numberReset) / 2 * (numberResetMs - numberTs) / 3600000;
            numberOld = numberReset;
            numberTs = numberResetMs;
        }
        const dateTs = new Date(numberTs);
        stringA = dateTs.getFullYear().toFixed(0) + '-' + (dateTs.getMonth() + 1).toFixed(0).padStart(2, '0') + '-' + dateTs.getDate().toFixed(0).padStart(2, '0') + ' ';
        stringA = stringA + dateTs.getHours().toFixed(0).padStart(2, '0') + ':' + dateTs.getMinutes().toFixed(0).padStart(2, '0') + ':' + dateTs.getSeconds().toFixed(0).padStart(2, '0') + ': ';
        if (await ecoflowCatshape.objectExists(stringDeviceId + '.name')) {
            stringA = stringA + (await ecoflowCatshape.getStateAsync(stringDeviceId + '.name')).val + ': ';
        }
        ecoflowCatshape.log.info(stringA + stringId + ': ' + numberVal.toFixed(1));
        numberVal = 0;
    }
    if (boolOnline) {
        numberVal = numberVal + (numberOld + numberNew) / 2 * (numberNewPerMs - numberTs) / 3600000;
    }
    ecoflowCatshape.setState(stringId, {val: numberVal, ack: true, ts: numberNewPerMs});
}
