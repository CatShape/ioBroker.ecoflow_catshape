...
async function sendDeviceState(ecoflowCatshape, stringId, objState) {
    ...
    if (objEcoflowApi.hasOwnProperty('valuesForSetValueData')) {
        for (stringA in objEcoflowApi.valuesForSetValueData) {
            ...
            if (await ecoflowCatshape.objectExists(stringTempId)) {
                anyValue = getApiValueFromStateValueWithObject((await ecoflowCatshape.getStateAsync(stringTempId)).val, await ecoflowCatshape.getObjectAsync(stringTempId));
    ...
    anyValue = getApiValueFromStateValue(objState.val, objEcoflowApi);
            ...
            if (objStateObj.native.ecoflowApi.hasOwnProperty('quotaValueKey')) {
                ...
                if (anyValue === null) {
                    ...
                    if ((objData.online == 1) && objStateObj.native.hasOwnProperty('cumulateDailyByTimeId')) {
                        updateCumulateDailyByTime(ecoflowCatshape, stringDeviceId + '.' + objStateObj.native.cumulateDailyByTimeId
...
function getApiValueFromStateValueWithObject(anyValue, objStateObj) {
    
    let anyRet = anyValue;
    
    if (objStateObj.native.hasOwnProperty('ecoflowApi')) {
        anyRet = getApiValueFromStateValue(anyValue, objStateObj.native.ecoflowApi);
    }
    return anyRet;
}
...
async function ecoflowRequest(ecoflowCatshape, objConfig) {
    ...
    try {
        ...
        if (error.response) {
            ecoflowCatshape.log.info('error.response.status: ' + error.response.status);
            ecoflowCatshape.log.info('error.response.headers: ' + JSON.stringify(error.response.headers));
            ecoflowCatshape.log.info('error.response.data: ' + JSON.stringify(error.response.data));
        } else if (error.request) {
            ecoflowCatshape.log.info('error.request: ' + error.request);
        } else {
            ecoflowCatshape.log.info('error.message: ' + error.message);
        }
        throw error;
    }
    return objRet;
